    ...
    if check_year == 'NO ACCOUNT' or check_year == 'CLOSE':
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                check_date = form.cleaned_data['cheque_date']
                ...
                item.save()    
                Payables.objects.get_or_create(pv_id = item, amount=total,devision= item.devision,sub_division=item.sub_division,acoounting_year=item.acoounting_year)
...
def pull_beneficiaries(request,pv_id):
    ...
    if beneficiary:
        for i in beneficiary:
            PaymentVoucherBeneficiary.objects.get_or_create(pv_id=item,staff=i.staff,amount=i.amount,ref=document.id)
    elif document.approved_budget > 1.00 and not beneficiary:
        PaymentVoucherBeneficiary.objects.get_or_create(pv_id=item,staff=document.staff_from,amount=document.approved_budget,ref=document.id)
    else:
        pass
                ...
                try:
                    for i in dbframe.itertuples():
                        try:
                            ...
                            PaymentVoucherBeneficiary.objects.get_or_create(pv_id=pv,staff=staff,amount=i.Amount)
