...
class ChequeForm(forms.Form):
    cheque_date = forms.DateField(widget=NumberInput(attrs={'type': 'date'}),label=False,required=True)
    cheque_number = forms.CharField(required=False,label=False)
...
class InternalAuditForm(forms.ModelForm):
    ...
    ia_code = forms.CharField(label=False)
    ...
    class Meta:
        model = Pveligibility
        fields = ('ia_date','ia_code','source_of_funding','cost_center','description','status','remarks',)
